...
//					std::cout << "DIST: " << direction << std::endl;
					...
					usleep(500);	//500
					if(gpioRead(limit) == HIGH){										//オーバラン(リミットが反応）したときに止める
						printf("limit\n");
//						std::cout << "mallet_X2: " << mallet_X << std::endl;
//						std::cout << "mallet_Y2: " << mallet_Y << std::endl;
						usleep(1);
...
//							printf("D\n");
						...
						f = gpioHardwarePWM(motorY_pulse, 0, 500000);
						preFrequencyX = 200;
						frequencyX = preFrequencyX;
						preFrequencyY = 200;
						frequencyY = preFrequencyY;
					}
		...
		if(gpioRead(rasp_sig)== set_mode){
			...
			f = gpioHardwarePWM(motorX_pulse, 100, 500000);
			usleep(500000);
			f = gpioHardwarePWM(motorX_pulse, 0, 500000);
