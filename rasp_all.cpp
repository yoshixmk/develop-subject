...
int main(int argc, char* argv[]) 
{
	...
	//原点復帰
	gpioSetMode(origin_x, PI_INPUT);
	
	gpioSetMode(origin_y, PI_INPUT);
	...
	gpioWrite(13, 0);	//外部BGM:OFF
	gpioWrite(26, 0);	//外部BGM:OFF
	/*
	while(1){
...
/*			gpioWrite(25, 0);	//X
			...
			gpioWrite(7, 1);	//後
			gpioWrite(24, 0);
			while(gpioRead(origin_y) == HIGH){
					f = gpioHardwarePWM(19, 100, 500000);
					while(gpioRead(origin_y) == HIGH);
			}
